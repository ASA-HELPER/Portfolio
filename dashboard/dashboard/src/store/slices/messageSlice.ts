...
export const getAllMessages = () => async (dispatch:AppDispatch) => {
  dispatch(messageSlice.actions.getAllMessagesRequest());
  try {
    const response = await axios.get(
      "https://portfolio-13of.onrender.com/api/v1/message/getall",
...
export const deleteMessage = (id:string) => async (dispatch:AppDispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://portfolio-13of.onrender.com/api/v1/message/delete/${id}`,
      {
        withCredentials: true,
      }
    );
